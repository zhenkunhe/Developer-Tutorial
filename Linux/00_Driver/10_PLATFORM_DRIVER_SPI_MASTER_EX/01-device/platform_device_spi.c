...
#define MY_DEV_NAME "bcm2708_spi_my"
...
static int __init my_bcm2708_spi_init(void)
{
	...
	printk("my_bcm2708_spi_init\n");
	
	bcm2709_register_clkdev(clk, "bcm2708_spi_my.0");		
	ret = platform_device_register(&bcm2708_spi_device);
	if (ret)
		printk("Unable to register platform device : %d\n",ret);
