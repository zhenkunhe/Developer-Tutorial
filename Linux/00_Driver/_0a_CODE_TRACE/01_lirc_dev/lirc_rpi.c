...
// 這是個模組內限定的function , 從名稱來看應該是設定一些時機上的
// 初始設定值
	...
	unsigned int new_freq)
{
	...
	// duty_cycle, 設定為50 (%?) 是個模組內限定的設定值
	// freq 同上，預設 38000 (38kHz)
	duty_cycle = new_duty_cycle;
	freq = new_freq;
	// period 先猜是整個ir訊號的時間長度
	period = 1000 * 1000000L / freq;
	// 計算大約的pulse 和space 的時間寬度
...
static int set_use_inc(void *data)
{
	...
	// 向kernel 申請一個 gpio 的中斷, 將其中斷和irq_handler() 
	...
	// 初始化一些接收的IR訊號的時間特性pulse ..space width
...
static const struct file_operations lirc_fops = {
	...
	.read		= lirc_dev_fop_read,   // 已經實作在lirc_dev.c 沒有我們的事
...
static struct lirc_driver driver = {
	...
	.minor		= -1,
	.code_length	= 1,
	.sample_rate	= 0, // 取樣頻率? 是怎樣?打算用polling來做嗎
	                     // 設為0的話，表示不會用polling的方式來做?
	.data		= NULL,
	.add_to_buf	= NULL,
	.rbuf		= &rbuf,
...
static int __init lirc_rpi_init_module(void)
{
	...
	// 注意，driver並沒有設定sample_rate的值，所以也不用設定
	// add_to_buf
