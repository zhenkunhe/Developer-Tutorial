 ...
 */
#if 1
  #define dprint(fmt,s...) printk("collin-board,bcm2708.c: %s,%d:"fmt,__FUNCTION__,__LINE__,##s)
#else
  #define dprint(fmt,s...)
#endif  
 ...
 *  blog:
 *  email:
 *  see KernelAPI/driver-example/rasp_gpio/09_SPI_FLASH_MTD/01-spi-device-mtd/m25p80dev.c
 *
 * */
...
{
    dprint("\nDo init work for EEPROM\n");
}

static struct at24_platform_data bcm2708_eeprom_info = {
    .byte_len	=	(2*1024)/8,
    .page_size	=	8,
...
void __init bcm2708_init(void)
{
 ...
 //   dprint("register spi device \n");	
	...
	if (vc_i2c_override) {
	    ...
	    //register at24 device
	    dprint("register at24 device \n");
...
#ifdef CONFIG_BCM2708_SPIDEV
	dprint("register spi device --joseph\n");	
