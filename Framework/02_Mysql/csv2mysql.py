...
def GetDBColumns(cursor, db ,table):
    """
    generate the table columns string here, excluding the ones mentioned in the config file.
    """
    query = (
    "select column_name from information_schema.columns where table_name = '{}' and TABLE_SCHEMA='{}' order by ordinal_position".format(table,db))
...
def GetDBColumnIndex(cursor, db ,table, number=1):
    # get the number of column. return text.
    cols = GetDBColumns(cursor, db ,table)
    ret = cols.split(",")
    number = int(number) - 1
    return ret[number]
...
def MakeMenuDB(cursor, db, table):
    ...
    while choice != 0:
        ...
        print "\n\n\n\n\n\n\n\n\n\n\n\n\n\n"
        if choice != 0:
            test = GetDBColumnIndex(cursor, db , table, choice)
            test = str(test)
